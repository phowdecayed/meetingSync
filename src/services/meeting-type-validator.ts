...
export class MeetingTypeValidatorImpl implements MeetingTypeValidator {
        ...
        default:
          return {
            ...
            conflicts: [{
              ...
              suggestions: ['Select a valid meeting type (offline, hybrid, or online)']
            }],
  ...
  validateOfflineMeeting(data: MeetingFormData): ValidationResult {
    ...
    return {
      isValid: conflicts.filter(c => c.severity === ConflictSeverity.ERROR).length === 0,
      conflicts,
  ...
  private validateBasicMeetingData(data: MeetingFormData): ValidationResult {
    ...
    if (!data.duration || data.duration <= 0) {
      conflicts.push({
        ...
        suggestions: ['Enter a valid duration in minutes (e.g., 60 for 1 hour)']
      })
  ...
  private validateZoomRequirement(data: MeetingFormData): ValidationResult {
    ...
    if (data.isZoomMeeting) {
      ...
      if (data.zoomPassword && data.zoomPassword.length > 0) {
        if (data.zoomPassword.length < 4) {
          conflicts.push({
            type: ConflictType.INVALID_TYPE,
            severity: ConflictSeverity.WARNING,
            message: 'Zoom password should be at least 4 characters long for security.',
      ...
      this.checkBasicZoomAvailability().then(hasAccounts => {
        if (!hasAccounts) {
          conflicts.push({
            type: ConflictType.ZOOM_CAPACITY,
            severity: ConflictSeverity.ERROR,
            message: 'No Zoom accounts are available for scheduling meetings.',
            suggestions: [
