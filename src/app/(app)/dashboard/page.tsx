...
export default async function DashboardPage() {
  ...
  const totalDurationThisWeek = meetingsThisWeek.reduce((acc, m) => acc + m.duration, 0);
  ...
  const chartData = nextSevenDays.map(day => ({
    date: format(day, 'EEE'),
    meetings: userMeetings.filter(m => isSameDay(new Date(m.date), day)).length,
  }));
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">My Upcoming Meetings</CardTitle>
            <Calendar className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{upcomingMeetings.length}</div>
            <p className="text-xs text-muted-foreground">Your scheduled upcoming meetings</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">My Meetings this Week</CardTitle>
            <BarChartIcon className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalMeetingsThisWeek}</div>
            <p className="text-xs text-muted-foreground">Meetings you are attending or organizing</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Time in Meetings (My Week)</CardTitle>
            <Clock className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(totalDurationThisWeek / 60).toFixed(1)} hours</div>
            <p className="text-xs text-muted-foreground">Your total scheduled meeting time</p>
          </CardContent>
        </Card>
      </div>
