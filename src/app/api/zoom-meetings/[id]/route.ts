...
export async function GET(
  ...
  try {
    ...
    const participants = meeting.participants ? meeting.participants.split(',').map(p => p.trim()) : [];
...
export async function DELETE(
  ...
  try {
    ...
    if (!isOrganizer && !isAdmin) {
      return NextResponse.json({ error: 'Forbidden: You are not authorized to delete this meeting.' }, { status: 403 });
    }
