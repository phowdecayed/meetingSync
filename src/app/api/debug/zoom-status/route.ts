...
function generateRecommendations(
  ...
  if (!health.zoomCredentials.configured) {
    recommendations.push(
      'Configure Zoom credentials in Settings to enable Zoom meeting features',
    )
  }

  if (stats.zoomAccounts === 0) {
    recommendations.push(
      'Add at least one Zoom account to enable concurrent meeting management',
    )
  }

  if (stats.zoomAccounts === 1) {
    recommendations.push(
      'Consider adding more Zoom accounts to increase concurrent meeting capacity (currently limited to 2 concurrent meetings)',
    )
  }

  if (stats.meetingRooms === 0) {
    recommendations.push(
      'Add meeting rooms to enable room booking and conflict detection',
    )
  }

  if (
    health.overall.errors.length === 0 &&
